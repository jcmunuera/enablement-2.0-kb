...
**Master Document:** See `ENABLEMENT-MODEL-v1.2.md` for complete conceptual model
...
**Example:** `mod-001-circuit-breaker.../validation/circuit-breaker-check.sh`
...
| Module | Validates |
|--------|-----------|
| `mod-001-circuit-breaker-*` | Circuit breaker configuration |
| `mod-015-hexagonal-base-*` | Hexagonal architecture structure |
...
source ../../modules/mod-015-hexagonal.../validation/hexagonal-structure-check.sh "$SERVICE_DIR" "$BASE_PACKAGE_PATH"

if feature_enabled "circuit_breaker" "$INPUT_JSON"; then
    source ../../modules/mod-001-circuit-breaker.../validation/circuit-breaker-check.sh "$SERVICE_DIR"
fi
```
...
| Tier 3 | Embedded in module | `modules/mod-001-.../validation/` | CODE only |
...
if feature_enabled "circuit_breaker"; then
    run_module_validator "mod-001-circuit-breaker"
fi
```
...
- **ENABLEMENT-MODEL-v1.2.md** - Complete conceptual model
...
```
ENABLEMENT-MODEL-v1.2.md (Master)
    ↓
