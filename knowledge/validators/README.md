...
bash "$MODULES/mod-015-hexagonal-base-java-spring/validation/hexagonal-check.sh" "$PROJECT_DIR"
TOTAL_ERRORS=$((TOTAL_ERRORS + $?))

# Tier-3 Module (conditional based on features)
if [ -f "$PROJECT_DIR/src/main/java/"**/CircuitBreakerConfig.java ]; then
    bash "$MODULES/mod-001-circuit-breaker-java-resilience4j/validation/circuit-breaker-check.sh" "$PROJECT_DIR"
    TOTAL_ERRORS=$((TOTAL_ERRORS + $?))
fi
